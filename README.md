...
| 修复 | 防火墙 IPv6 放行 | 放行规则恢复生效，不再误用 IPv4 目标端口；按端口放行（TCP + UDP）而不限定 IPv6 网段；只开 IPv6 放行、没配 IPv4 转发时也能生效（目标回退 lan）；`firewall reload` 失败自动重试、重试仍失败再改用 `restart`，并把命令自身的报错写进日志；成功后会到内核规则集里反查 fw4 打的 `!fw4: <段名>` 标记 —— 命令返回 0 却其实没生效这种情况，日志里能直接看出来（fw4 状态文件缺失、或撞上它自身 restart 的窗口时 `reload` 会瞬时失败） |
| 修复 | 服务启动 | 「自定义脚本路径」指向一个已删除的文件时，不再让整个实例校验失败、静默不启动（改为运行时检查该文件并写日志） |
| 修复 | 日志安全 | 配置校验失败时不再把该实例的全部配置值（Telegram token、各类密码）写进系统日志 |
| 修复 | qBittorrent 端口同步 | 响应体与 HTTP 状态码分开比较，端口已改成功时不再被误判失败并重试到上限 |
...
> 版本说明：`1.6.0` 汇总了此前 `1.5.11` ~ `1.5.13` 的全部改动，安装 `1.6.0` 即可获得以上全部修复与新增（无需逐个安装中间版本）。
