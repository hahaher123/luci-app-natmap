...
| 修复 | 防火墙 IPv6 放行 | 放行规则恢复生效，不再误用 IPv4 目标端口；按端口放行（TCP + UDP）而不限定 IPv6 网段；只开 IPv6 放行、没配 IPv4 转发时也能生效（目标回退 lan）；`firewall reload` 失败时自动改用 `restart` 并把命令自身的报错写进日志（fw4 状态文件缺失时 `reload` 必然失败、规则永远落不到内核） |
