...
while (true); do
  ...
  if [ -n "$cloudflare_ruleset_id" ]; then
    ...
    if [ -z "$rule_idx" ]; then
      echo "$(TZ='CST-8' date +'%Y-%m-%d %H:%M:%S') : $GENERAL_NAT_NAME - $LINK_MODE 未找到名为 $LINK_CLOUDFLARE_ORIGIN_RULE_NAME 的规则" >>/var/log/natmap/natmap.log
      echo "$(TZ='CST-8' date +'%Y-%m-%d %H:%M:%S') : $GENERAL_NAT_NAME - $LINK_MODE 未找到名为 $LINK_CLOUDFLARE_ORIGIN_RULE_NAME 的规则"
    else
      ...
      if [ "$(echo "$result" | jq -r '.success')" == "true" ]; then
        echo "$(TZ='CST-8' date +'%Y-%m-%d %H:%M:%S') : $GENERAL_NAT_NAME - $LINK_MODE 修改成功: origin port -> $outter_port" >>/var/log/natmap/natmap.log
        echo "$(TZ='CST-8' date +'%Y-%m-%d %H:%M:%S') : $GENERAL_NAT_NAME - $LINK_MODE 修改成功: origin port -> $outter_port"
        break
      else
