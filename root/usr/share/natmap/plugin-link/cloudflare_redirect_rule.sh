...
while (true); do
  ...
  if [ "$(printf '%s' "$currrent_rule" | jq -r '.success' 2>/dev/null)" == "true" ]; then
    ...
    rule_idx=$(printf '%s' "$currrent_rule" | jq -r --arg name "$LINK_CLOUDFLARE_REDIRECT_RULE_NAME" '.result.rules | to_entries | map(select(.value.description == $name) | .key) | first // empty' 2>/dev/null)

    if [ -z "$rule_idx" ]; then
      log "未找到名为 $LINK_CLOUDFLARE_REDIRECT_RULE_NAME 的规则"
    else
      # 替换 NEW_PORT 占位符为当前打洞端口
      redirect_rule_target_url=$(printf '%s' "$LINK_CLOUDFLARE_REDIRECT_RULE_TARGET_URL" | sed 's/NEW_PORT/'"$outter_port"'/g')

      # 统一写入 target_url.value（静态跳转形式，接受纯 URL）。
      # 注意：动态跳转规则(target_url.expression)不接受裸 URL 字面量，
      # 若原规则是动态类型，这里整体替换 target_url 会自动转为静态类型。
      new_rule=$(printf '%s' "$currrent_rule" | jq --arg url "$redirect_rule_target_url" ".result.rules[$rule_idx].action_parameters.from_value.target_url = {\"value\": \$url}")
      ...
      if [ "$(printf '%s' "$result" | jq -r '.success' 2>/dev/null)" == "true" ]; then
        log "修改成功: $redirect_rule_target_url"
        break
      else
